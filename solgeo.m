...
LATd = Et + 4*(lon-lsm); % Iqbal(1.4.2) difference to local apparent time
time_LT = time + minutes(LATd); % local apparent time
hro = hour(time_LT)+minute(time_LT)/60; % decimal hours of LAT
omega = (hro-12).*deg2rad(15); % hour angle 0 = 12h LAT
latrad = deg2rad(lat);
sunel = asin(sin(latrad).*sin(delta) + cos(latrad).*cos(delta).*cos(omega)); % sun elevation
TOA = Io.*Eo.*sin(sunel); % Top of atmosphere radiation
omegas = acos(tan(latrad)*-1.*tan(delta));
omegas(omegas~=real(omegas)) = NaN;
LOD = 2/15.*rad2deg(omegas);
